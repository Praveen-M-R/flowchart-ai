...
class Requester:
    ...
    def add_parameters_to_url(
        ...
        parameter_list = [
            (key, value) for key, values in url_params.items() for value in values
        ]
    ...
    def graphql_node(
        self, node_id: str, graphql_schema: str, node_type: str
    ) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        """
        :calls: `POST /graphql <https://docs.github.com/en/graphql>`_
        """
    ...
    def createException(
        ...
        if status == 401 and lc_message == "bad credentials":
            ...
            lc_message == "not found" or "no object found" in lc_message
        ):
    ...
    def __makeAbsoluteUrl(self, url: str) -> str:
        ...
        if url.startswith("/"):
            ...
            assert o.path.startswith(
                (self.__prefix, self.__graphql_prefix, "/api/", "/login/oauth")
            ), o.path
