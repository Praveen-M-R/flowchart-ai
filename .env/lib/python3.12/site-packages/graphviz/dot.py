...
class Dot(quoting.Quote, base.Base):
    ...
    @_tools.deprecate_positional_args(
        supported_number=2, ignore_arg="self", category=DeprecationWarning
    )
    def node(
        self, name: str, label: typing.Optional[str] = None, _attributes=None, **attrs
    ) -> None:
        """Create a node.
    ...
    @_tools.deprecate_positional_args(
        supported_number=3, ignore_arg="self", category=DeprecationWarning
    )
    def edge(
        self,
        tail_name: str,
    ...
    @_tools.deprecate_positional_args(
        supported_number=1, ignore_arg="self", category=DeprecationWarning
    )
    def attr(self, kw: typing.Optional[str] = None, _attributes=None, **attrs) -> None:
        ...
        if kw is not None and kw.lower() not in ("graph", "node", "edge"):
            raise ValueError(
                "attr statement must target graph, node, or edge:" f" {kw!r}"
            )
    ...
    def subgraph(
        ...
        if graph.directed != self.directed:
            raise ValueError(
                f"{self!r} cannot add subgraph of different kind:" f" {graph!r}"
            )
